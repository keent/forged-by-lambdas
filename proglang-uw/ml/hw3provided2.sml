...
val longest_string4 = longest_string_helper (fn (a,b) => b >= a)
...
fun match value pattern = 
  ...
  | (Tuple v::vs, TupleP p::ps) => if List.length(vs) = List.length(ps) andalso match v p andalso
				   isSome (all_answers((fn (v,p) => match v p), ListPair.zip(vs, ps)))
			           then (SOME (p, v)) @ match vs ps
			           else SOME [] 
  | (Constructor(s,v::vs), ConstructorP(s1,p::ps))  => if s = s1 andalso isSome (match p v)
					       then (SOME(p,v)) @ match vs ps
					       else SOME []
  | _ => NONE
